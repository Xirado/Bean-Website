<!--Quick note: This file isn't actually being used anywhere, I just don't want to delete it just incase its used again the the future-->
<!--Also apparently theres an error so I commeneted everything out-->
...
export default {
	...
	created() {
		this.supportingCommandList = Object.keys(this.commandList).concat(
	...
	methods: {
		...
		handleCommand(e) {
			...
			if (commandArr[0] === "help") {
				...
				this.commandList[this.inputCommand].messages.map((item) =>
		...
		handlekeyEvent(e) {
			...
			switch (e.keyCode) {
				case 38:
					this.historyIndex =
						this.historyIndex === 0 ? 0 : this.historyIndex - 1;
		...
		handleRun(taskName, input) {
			if (!this.taskList[taskName] || !this.taskList[taskName][taskName])
				return Promise.resolve();
			this.lastLineContent = "...";
			return this.taskList[taskName][taskName](this.pushToList, input)
				.then((done) => {
		...
		autoScroll() {
			this.$nextTick(() => {
				this.$refs.terminalWindow.scrollTop =
					this.$refs.terminalLastLine.offsetTop;
			});
		},
	},
};
