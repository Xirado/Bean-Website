<template>
  <nav>
    <v-app-bar flat color="#7630ff" clipped-left app>
        ...
        <template v-slot:activator="{ on }">
          <v-btn icon x-large v-on="on" class="mx-1">
            <v-avatar color="brown" size="48">
              <img :src="user.user.effective_avatar">
            </v-avatar>
          </v-btn>
        </template>
        <v-card>
          <v-list-item-content class="justify-center">
            <div class="mx-auto text-center">
              ...
              <h3>{{ user.user.username }}<span class="ml-1 grey--text">#{{ user.user.discriminator }}</span>
              </h3>
        ...
        <v-navigation-drawer stateless v-model="drawer" clipped app color="#23272a">
      <v-list>
...
<script>
import { logout, fetchLoginURL, fetchInviteURL, fetchUser } from '@/discord/discord.js'
